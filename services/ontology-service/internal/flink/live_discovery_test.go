...
func TestPlanFollowUpLiveRequestsDiscoversJiraRemoteLinksFromSearchSnapshots(t *testing.T) {
	...
	if requests[0].SourceObjectKind != "jira_remote_links" || requests[0].SourceObjectID != "FLINK-1" {
		t.Fatalf("first request = %#v", requests[0])
	}
	if requests[1].SourceObjectKind != "jira_remote_links" || requests[1].SourceObjectID != "FLINK-2" {
