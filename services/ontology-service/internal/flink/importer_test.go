...
func TestFixtureImporterWritesSnapshotsBeforeMappedBatches(t *testing.T) {
	...
	importer := NewFlinkFixtureImporter(writer)

	result, err := importer.Import(ctx, FlinkFixtureImportConfig{
...
func TestFixtureImporterReplaysThroughEntStoreIdempotently(t *testing.T) {
	...
	importer := NewFlinkFixtureImporter(store)
	cfg := FlinkFixtureImportConfig{
		FixtureDir:   "testdata/flink-fixture",
		SnapshotRoot: t.TempDir(),
		Now:          time.Date(2026, 6, 7, 12, 0, 0, 0, time.UTC),
	}
	...
	assertObject(t, graph, "pr:apache/flink-kubernetes-operator#1234")
	assertObject(t, graph, "document:apache/flink-kubernetes-operator:docs/content/docs/custom-resource/autoscaler.md")
	assertAssociation(t, graph, ontology.AssocImplementedBy)
	assertAssociation(t, graph, ontology.AssocChangesFile)
	assertAssociation(t, graph, ontology.AssocDiscussedIn)
}
...
func assertAssociation(t *testing.T, graph domain.Neighborhood, associationType domain.AssociationType) {
	t.Helper()
