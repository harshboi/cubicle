...
func (s *WorkstreamService) Overview(ctx context.Context, slug string) (WorkstreamOverview, error) {
	...
	for _, object := range graph.Objects {
		...
		case ontology.ObjectWorkstream:
			if object.Key == key {
				overview.Workstream = object
			}
		case ontology.ObjectTicket:
			overview.Tickets = append(overview.Tickets, object)
		case ontology.ObjectPullRequest:
			overview.PullRequests = append(overview.PullRequests, object)
		case ontology.ObjectCodeFile:
			overview.CodeFiles = append(overview.CodeFiles, object)
		case ontology.ObjectBlocker:
			overview.Blockers = append(overview.Blockers, object)
		case ontology.ObjectActionCandidate:
			overview.ActionCandidates = append(overview.ActionCandidates, object)
		}
	}
