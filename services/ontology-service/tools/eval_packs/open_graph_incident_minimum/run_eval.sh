...
OUT_DIR=${OUT_DIR:-/tmp/open_graph_incident_minimum}
DB="$OUT_DIR/open_graph_incident.db"
...
for forbidden in [
    ...
    if forbidden in text:
        raise SystemExit("open graph context leaked " + forbidden)
PY
}
...
go run ./cmd/ontology-service bounded-graph-context-export \
  ...
  --start-key customer:acme \
  --association-types affected_by,mitigated_by,updated_in \
  --depth 2 \
  --limit-per-object 3 \
  --out "$context_json"
    ...
    "answers": [
        {
            ...
            "label": "Depth-2 open Ent graph traversal",
            "path": depth2_path,
            "strategy": "open_ent_bounded_graph_context_depth_2",
