...
OUT_DIR="${OUT_DIR:-/tmp/real_github_issue_pr_minimum_mlx}"
DB="$OUT_DIR/real_github_issue_pr.db"
SOURCE_AUTHORITY_JSON="$SCRIPT_DIR/source_authority.json"
OPEN_GRAPH_FIXTURE_JSON="$SCRIPT_DIR/open_graph_fixture.json"
GOLDEN_JSON="$SCRIPT_DIR/golden_questions.json"
...
assert_no_leakage() {
  ...
  .venv/bin/python - "$context_path" <<'PY'
import json
import sys
...
for forbidden in [
    ...
    if forbidden in text:
        raise SystemExit("real GitHub LLM context leaked " + forbidden)
PY
}
...
go run ./cmd/ontology-service bounded-graph-context-export \
  ...
  --start-object-type github_issue \
  --start-key github:issue:cli/cli#13262 \
  --association-types closed_by,opened_by,authored_by \
  --depth 2 \
  --limit-per-object 4 \
  --out "$context_json"
    ...
    "answers": [
        {
            ...
            "path": raw_path,
            "strategy": "real_github_open_graph_raw_model",
            "answer_kind": "raw",
        },
        {
            "key": "repaired_model",
            ...
            "path": repaired_path,
            "strategy": "real_github_open_graph_repaired_model",
        ...
        },
        {
            ...
            "path": generic_path,
            "strategy": "real_github_open_graph_generic_baseline",
            "answer_kind": "candidate",
        },
        {
            "key": "depth_0_seed",
            "label": "Seed-only GitHub issue",
            "path": seed_only_path,
            "strategy": "real_github_open_graph_depth_0",
            "answer_kind": "baseline",
        },
        {
            "key": "typed_rows",
            "label": "GitHub open object rows without associations",
            "path": typed_row_path,
...
for label in ["raw", "repaired", "generic_baseline", "seed_only", "typed_row"]:
...
if raw:
    print("real_github_issue_pr_llm raw_unknown_citations=" + json.dumps(raw.get("unknown_citations", [])))
    print("real_github_issue_pr_llm raw_policy_violations=" + json.dumps(raw.get("citation_policy_violations", [])))
    print("real_github_issue_pr_llm raw_unsupported_statement_count=" + str(raw.get("unsupported_statement_count")))
...
for gate in comparison.get("promotion_gates", []):
    ...
    if not gate.get("passes"):
        failures.append("promotion gate failed: " + str(gate.get("key")))
