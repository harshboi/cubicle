...
const (
	...
	EdgeTicketDocuments = "ticket_documents"
	...
	DocumentsTable = "ticket_documents"
	...
	// TicketDocumentsTable is the table that holds the ticket_documents relation/edge.
	TicketDocumentsTable = "ticket_documents"
	// TicketDocumentsInverseTable is the table name for the TicketDocument entity.
	// It exists in this package in order to avoid circular dependency with the "ticketdocument" package.
	TicketDocumentsInverseTable = "ticket_documents"
	// TicketDocumentsColumn is the table column denoting the ticket_documents relation/edge.
	TicketDocumentsColumn = "ticket_id"
...
var (
	...
	DocumentsPrimaryKey = []string{"ticket_id", "document_id"}
...
func DeletionStateValidator(ds DeletionState) error {
	...
	case DeletionStatePresent, DeletionStateDeleted, DeletionStateUnknown:
		return nil
	default:
		return fmt.Errorf("ticket: invalid enum value for deletion_state field: %q", ds)
	}
}
...
func ACLStateValidator(as ACLState) error {
	...
	case ACLStateUnknown, ACLStateCurrent, ACLStateStale, ACLStateBlocked, ACLStateUnavailable:
		return nil
	default:
		return fmt.Errorf("ticket: invalid enum value for acl_state field: %q", as)
	}
}
...
// ByTicketDocumentsCount orders the results by ticket_documents count.
func ByTicketDocumentsCount(opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborsCount(s, newTicketDocumentsStep(), opts...)
	}
}

// ByTicketDocuments orders the results by ticket_documents terms.
func ByTicketDocuments(term sql.OrderTerm, terms ...sql.OrderTerm) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newTicketDocumentsStep(), append([]sql.OrderTerm{term}, terms...)...)
	}
}
...
func newTicketPullRequestsStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(TicketPullRequestsInverseTable, FieldID),
		sqlgraph.Edge(sqlgraph.O2M, true, TicketPullRequestsTable, TicketPullRequestsColumn),
	)
}
func newTicketDocumentsStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(TicketDocumentsInverseTable, FieldID),
