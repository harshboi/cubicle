...
type EvidenceQuery struct {
	...
	withSupersededByEvidence *EvidenceQuery
	withSupersededEvidence   *EvidenceQuery
...
// QuerySupersededByEvidence chains the current query on the "superseded_by_evidence" edge.
func (_q *EvidenceQuery) QuerySupersededByEvidence() *EvidenceQuery {
...
// QuerySupersededEvidence chains the current query on the "superseded_evidence" edge.
func (_q *EvidenceQuery) QuerySupersededEvidence() *EvidenceQuery {
	query := (&EvidenceClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *EvidenceQuery) Clone() *EvidenceQuery {
	...
	return &EvidenceQuery{
		...
		withSupersededByEvidence: _q.withSupersededByEvidence.Clone(),
		withSupersededEvidence:   _q.withSupersededEvidence.Clone(),
...
// WithSupersededByEvidence tells the query-builder to eager-load the nodes that are connected to
// the "superseded_by_evidence" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *EvidenceQuery) WithSupersededByEvidence(opts ...func(*EvidenceQuery)) *EvidenceQuery {
	query := (&EvidenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSupersededByEvidence = query
	return _q
}

// WithSupersededEvidence tells the query-builder to eager-load the nodes that are connected to
// the "superseded_evidence" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *EvidenceQuery) WithSupersededEvidence(opts ...func(*EvidenceQuery)) *EvidenceQuery {
	query := (&EvidenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSupersededEvidence = query
	return _q
}
...
func (_q *EvidenceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Evidence, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withSupersededByEvidence != nil,
			_q.withSupersededEvidence != nil,
		}
	)
	...
	if query := _q.withSupersededByEvidence; query != nil {
		if err := _q.loadSupersededByEvidence(ctx, query, nodes, nil,
			func(n *Evidence, e *Evidence) { n.Edges.SupersededByEvidence = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSupersededEvidence; query != nil {
		if err := _q.loadSupersededEvidence(ctx, query, nodes,
			func(n *Evidence) { n.Edges.SupersededEvidence = []*Evidence{} },
			func(n *Evidence, e *Evidence) { n.Edges.SupersededEvidence = append(n.Edges.SupersededEvidence, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EvidenceQuery) loadSupersededByEvidence(ctx context.Context, query *EvidenceQuery, nodes []*Evidence, init func(*Evidence), assign func(*Evidence, *Evidence)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Evidence)
	for i := range nodes {
		fk := nodes[i].SupersededByEvidenceID
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "superseded_by_evidence_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EvidenceQuery) loadSupersededEvidence(ctx context.Context, query *EvidenceQuery, nodes []*Evidence, init func(*Evidence), assign func(*Evidence, *Evidence)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(evidence.FieldSupersededByEvidenceID)
	}
	query.Where(predicate.Evidence(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(evidence.SupersededEvidenceColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.SupersededByEvidenceID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "superseded_by_evidence_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *EvidenceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSupersededByEvidence != nil {
			_spec.Node.AddColumnOnce(evidence.FieldSupersededByEvidenceID)
		}
	}
