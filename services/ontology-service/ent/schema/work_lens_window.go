...
// parent for every message a person ever wrote. Crawlers and GraphQL resolvers
	...
	return appendFields(
		...
		[]ent.Field{
			...
			field.String("checkpoint").
				Optional().
				Comment("Crawler or query cursor used to resume rebuilding this window."),
			field.Int("result_count").
				Default(0).
				Comment("Cached number of result rows assigned to this window."),
			field.Bool("is_complete").
				Default(false).
				Comment("Whether the backing crawler has fully populated this window."),
...
func (WorkLensWindow) Indexes() []ent.Index {
	return []ent.Index{
		...
		index.Fields("work_lens_id", "source", "window_start_at"),
	}
}
