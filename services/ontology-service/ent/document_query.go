...
// QueryFragments chains the current query on the "fragments" edge.
func (_q *DocumentQuery) QueryFragments() *DocumentFragmentQuery {
...
func (_q *DocumentQuery) QueryWorkPanes() *WorkPaneQuery {
...
// WithFragments tells the query-builder to eager-load the nodes that are connected to
// the "fragments" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *DocumentQuery) WithFragments(opts ...func(*DocumentFragmentQuery)) *DocumentQuery {
	query := (&DocumentFragmentClient{config: _q.config}).Query()
...
func (_q *DocumentQuery) WithWorkPanes(opts ...func(*WorkPaneQuery)) *DocumentQuery {
	query := (&WorkPaneClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if query := _q.withFragments; query != nil {
	...
	if query := _q.withWorkPanes; query != nil {
		if err := _q.loadWorkPanes(ctx, query, nodes,
			func(n *Document) { n.Edges.WorkPanes = []*WorkPane{} },
			func(n *Document, e *WorkPane) { n.Edges.WorkPanes = append(n.Edges.WorkPanes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DocumentQuery) loadFragments(ctx context.Context, query *DocumentFragmentQuery, nodes []*Document, init func(*Document), assign func(*Document, *DocumentFragment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(documentfragment.FieldDocumentID)
	}
	query.Where(predicate.DocumentFragment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(document.FragmentsColumn), fks...))
	}))
...
func (_q *DocumentQuery) loadWorkPanes(ctx context.Context, query *WorkPaneQuery, nodes []*Document, init func(*Document), assign func(*Document, *WorkPane)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(document.WorkPanesTable)
		s.Join(joinT).On(s.C(workpane.FieldID), joinT.C(document.WorkPanesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(document.WorkPanesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(document.WorkPanesPrimaryKey[1]))
