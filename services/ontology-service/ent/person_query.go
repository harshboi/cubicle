...
type PersonQuery struct {
	...
	withWorkSurfaces *WorkSurfaceQuery
...
// QueryWorkSurfaces chains the current query on the "work_surfaces" edge.
func (_q *PersonQuery) QueryWorkSurfaces() *WorkSurfaceQuery {
...
func (_q *PersonQuery) Clone() *PersonQuery {
	...
	return &PersonQuery{
		...
		withWorkSurfaces: _q.withWorkSurfaces.Clone(),
...
// WithWorkSurfaces tells the query-builder to eager-load the nodes that are connected to
// the "work_surfaces" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PersonQuery) WithWorkSurfaces(opts ...func(*WorkSurfaceQuery)) *PersonQuery {
	query := (&WorkSurfaceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withWorkSurfaces = query
	return _q
}
...
func (_q *PersonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Person, error) {
	...
	if query := _q.withWorkSurfaces; query != nil {
		if err := _q.loadWorkSurfaces(ctx, query, nodes,
			func(n *Person) { n.Edges.WorkSurfaces = []*WorkSurface{} },
			func(n *Person, e *WorkSurface) { n.Edges.WorkSurfaces = append(n.Edges.WorkSurfaces, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PersonQuery) loadWorkSurfaces(ctx context.Context, query *WorkSurfaceQuery, nodes []*Person, init func(*Person), assign func(*Person, *WorkSurface)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(worksurface.FieldPersonID)
	}
	query.Where(predicate.WorkSurface(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(person.WorkSurfacesColumn), fks...))
	}))
