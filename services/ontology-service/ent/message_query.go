...
func (_q *MessageQuery) QueryTickets() *TicketQuery {
...
func (_q *MessageQuery) QueryWorkPanes() *WorkPaneQuery {
...
func (_q *MessageQuery) WithTickets(opts ...func(*TicketQuery)) *MessageQuery {
	query := (&TicketClient{config: _q.config}).Query()
...
func (_q *MessageQuery) WithWorkPanes(opts ...func(*WorkPaneQuery)) *MessageQuery {
	query := (&WorkPaneClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := _q.withTickets; query != nil {
		if err := _q.loadTickets(ctx, query, nodes,
			func(n *Message) { n.Edges.Tickets = []*Ticket{} },
			func(n *Message, e *Ticket) { n.Edges.Tickets = append(n.Edges.Tickets, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withWorkPanes; query != nil {
		if err := _q.loadWorkPanes(ctx, query, nodes,
			func(n *Message) { n.Edges.WorkPanes = []*WorkPane{} },
			func(n *Message, e *WorkPane) { n.Edges.WorkPanes = append(n.Edges.WorkPanes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MessageQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*Message, init func(*Message), assign func(*Message, *Ticket)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(message.TicketsTable)
		s.Join(joinT).On(s.C(ticket.FieldID), joinT.C(message.TicketsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(message.TicketsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(message.TicketsPrimaryKey[1]))
...
func (_q *MessageQuery) loadWorkPanes(ctx context.Context, query *WorkPaneQuery, nodes []*Message, init func(*Message), assign func(*Message, *WorkPane)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Message)
	nids := make(map[int]map[*Message]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(message.WorkPanesTable)
		s.Join(joinT).On(s.C(workpane.FieldID), joinT.C(message.WorkPanesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(message.WorkPanesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(message.WorkPanesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
