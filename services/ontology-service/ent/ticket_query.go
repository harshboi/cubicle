...
func (_q *TicketQuery) QueryPullRequests() *PullRequestQuery {
...
func (_q *TicketQuery) QueryDocuments() *DocumentQuery {
...
func (_q *TicketQuery) QueryLatestEvidence() *EvidenceQuery {
...
// QueryTicketDocuments chains the current query on the "ticket_documents" edge.
func (_q *TicketQuery) QueryTicketDocuments() *TicketDocumentQuery {
	query := (&TicketDocumentClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TicketQuery) WithPullRequests(opts ...func(*PullRequestQuery)) *TicketQuery {
	query := (&PullRequestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TicketQuery) WithDocuments(opts ...func(*DocumentQuery)) *TicketQuery {
	query := (&DocumentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TicketQuery) WithLatestEvidence(opts ...func(*EvidenceQuery)) *TicketQuery {
	query := (&EvidenceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithTicketDocuments tells the query-builder to eager-load the nodes that are connected to
// the "ticket_documents" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TicketQuery) WithTicketDocuments(opts ...func(*TicketDocumentQuery)) *TicketQuery {
	query := (&TicketDocumentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTicketDocuments = query
	return _q
}
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if query := _q.withPullRequests; query != nil {
		if err := _q.loadPullRequests(ctx, query, nodes,
			func(n *Ticket) { n.Edges.PullRequests = []*PullRequest{} },
			func(n *Ticket, e *PullRequest) { n.Edges.PullRequests = append(n.Edges.PullRequests, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDocuments; query != nil {
		if err := _q.loadDocuments(ctx, query, nodes,
			func(n *Ticket) { n.Edges.Documents = []*Document{} },
			func(n *Ticket, e *Document) { n.Edges.Documents = append(n.Edges.Documents, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withTicketDocuments; query != nil {
		if err := _q.loadTicketDocuments(ctx, query, nodes,
			func(n *Ticket) { n.Edges.TicketDocuments = []*TicketDocument{} },
			func(n *Ticket, e *TicketDocument) { n.Edges.TicketDocuments = append(n.Edges.TicketDocuments, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TicketQuery) loadPullRequests(ctx context.Context, query *PullRequestQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *PullRequest)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Ticket)
	nids := make(map[int]map[*Ticket]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(ticket.PullRequestsTable)
		s.Join(joinT).On(s.C(pullrequest.FieldID), joinT.C(ticket.PullRequestsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(ticket.PullRequestsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(ticket.PullRequestsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *TicketQuery) loadDocuments(ctx context.Context, query *DocumentQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Document)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Ticket)
	nids := make(map[int]map[*Ticket]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(ticket.DocumentsTable)
		s.Join(joinT).On(s.C(document.FieldID), joinT.C(ticket.DocumentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(ticket.DocumentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(ticket.DocumentsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *TicketQuery) loadLatestEvidence(ctx context.Context, query *EvidenceQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Evidence)) error {
...
func (_q *TicketQuery) loadTicketPullRequests(ctx context.Context, query *TicketPullRequestQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *TicketPullRequest)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ticketpullrequest.FieldTicketID)
	}
...
func (_q *TicketQuery) loadTicketDocuments(ctx context.Context, query *TicketDocumentQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *TicketDocument)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Ticket)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ticketdocument.FieldTicketID)
	}
	query.Where(predicate.TicketDocument(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(ticket.TicketDocumentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TicketQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withLatestEvidence != nil {
			_spec.Node.AddColumnOnce(ticket.FieldLatestEvidenceID)
		}
	}
