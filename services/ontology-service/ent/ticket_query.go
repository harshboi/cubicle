...
func (_q *TicketQuery) QueryWorkPanes() *WorkPaneQuery {
...
func (_q *TicketQuery) WithWorkPanes(opts ...func(*WorkPaneQuery)) *TicketQuery {
	query := (&WorkPaneClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if query := _q.withWorkPanes; query != nil {
		if err := _q.loadWorkPanes(ctx, query, nodes,
			func(n *Ticket) { n.Edges.WorkPanes = []*WorkPane{} },
			func(n *Ticket, e *WorkPane) { n.Edges.WorkPanes = append(n.Edges.WorkPanes, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TicketQuery) loadWorkPanes(ctx context.Context, query *WorkPaneQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *WorkPane)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Ticket)
	nids := make(map[int]map[*Ticket]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(ticket.WorkPanesTable)
		s.Join(joinT).On(s.C(workpane.FieldID), joinT.C(ticket.WorkPanesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(ticket.WorkPanesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(ticket.WorkPanesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
